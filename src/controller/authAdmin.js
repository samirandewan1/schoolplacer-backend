...
export const authAdmin = async (req, res) => {
  try {
    ...
    if (!isMatch) return res.status(400).json({ status: "failure", message: "Wrong password" });
    const token = jwt.sign({ initiator: adminUser._id, role: "admin" }, config.SECRET.jwt_key, { expiresIn: "1d" });
