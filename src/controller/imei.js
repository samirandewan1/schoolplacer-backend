...
export const  assignImei = async(req, res) => {
  ...
  const { error, value } = assignImeiSchema.validate(imeaData, { abortEarly: false });
  if (error) {
  ...
  if(acknowledged){
      return res.status(200).json({ status: 'success', deviceId: insertedId });
    }
...
export const unassignImei = async(req, res) => {
  ...
  await actionLog(imeaData, initiator, role, 'unassignImei');
